...
field per paper; 9 fields, working spec §5.3, 2 shared with the imaging auditor —
preregistration, data_availability, see quality_fields.applies_to). Not a trimmed-down
trial auditor: attrition and confounding carry different weight when nothing was ever
randomised. The field list is identical across every cohort paper (it comes straight from
quality_fields, not anything paper-specific), so it's also the STATIC part of this prompt —
putting it in the system message, ahead of the paper's own full text in the user message,
means the prefix is byte-identical across different papers' audit calls, not just reused
within one call. That ordering is what makes provider-side prompt caching apply here at all
(see agents/base.py's module docstring on prefix ordering).
"""
...
SYSTEM_PROMPT_TEMPLATE = """You check whether an observational cohort paper's full text reports each of the
