"""Agent 2c — Qualitative auditor. See docs/agents.md §2c.

Ingest, one call per paper covering EVERY applicable field at once (v2 — was one call per
field per paper; 7 fields, working spec §5.3, including community involvement in design — a
methodological-quality field here, not an editorial add-on). The field list is identical
across every qualitative paper (it comes straight from quality_fields, not anything
paper-specific), so it's also the STATIC part of this prompt — putting it in the system
message, ahead of the paper's own full text in the user message, means the prefix is
...
SYSTEM_PROMPT_TEMPLATE = """You check whether a qualitative research paper's full text reports each of the
