...
class PracticalSupportTurn(BaseModel):
    """A real, practical need connected to being autistic/ADHD/etc. that isn't answerable
    from the literature and isn't a diagnostic ask either — workplace rights, education
    accommodations, benefits. `resources` always comes from a static, hand-maintained
    table (neurodiversity/practical_resources.py), never generated by a model — same
    principle as DistressTurn's crisis resources: a hallucinated organization name or URL
    here is close to the same failure class as a hallucinated crisis line."""
